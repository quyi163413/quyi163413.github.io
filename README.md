...
模块名称	模块说明
仪表盘	系统总览、全局数据可视化首页
频道列表	所有可播放频道统一展示、检索、编辑
固定源管理	用户手动指定持久留存的播放源维护
配置管理	系统参数、监控阈值、自动策略全局配置
质量趋势	源长期播放质量图表、波动记录查看

功能项	核心作用
成功率监控	实时统计各源播放请求成功占比
失败次数统计	分类记录连接超时、解码失败、404 等报错次数
崩溃风险分析	识别频繁掉线、瞬时断流高危源
自动回滚	候选版异常时自动切回稳定可用版本

源分层类型	状态标识	阶段定位	核心特性
稳定版	✅ 已验证稳定	输出使用	长期验证无故障，对外提供播放服务
候选版	⏳ 持续观察	观察验证中	新源短期测试，未完全达标，灰度观测
源池	待验证	新源发现	爬虫 / 接口自动抓取的全新未知源，待质检
固定源	🔒 永久保留	用户指定	手动添加锁定，不受自动清理、替换规则影响
